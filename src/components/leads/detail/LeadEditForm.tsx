...
// Esquema de validación para datos del contacto
...
interface LeadEditFormProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (data: CreateUpdateLeadDto) => Promise<boolean>;
  isUpdating: boolean;
  error: string | null;
}
...
export default function LeadEditForm({
  ...
  // Actualizar valores cuando cambia el lead
  useEffect(() => {
    if (lead && isOpen) {
      form.reset({
        firstName: lead.firstName,
  ...
  const onSubmit = async (values: ContactFormValues) => {
    // Convertir campos de formulario a DTO
    ...
    const success = await onUpdate(contactData);
    if (success) {
      onClose();
    }
  };
