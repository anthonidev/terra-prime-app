  ...
  if (!stages.length) {
    return (
      ...
      >
        ...
        <p className="text-muted-foreground">Este proyecto no tiene etapas registradas.</p>
      </motion.div>
    );
  }
    ...
    >
      {stages.map((stage, index) => (
        <motion.div key={stage.id} variants={item}>
          <Card className={!stage.isActive ? "opacity-90 border-border/50" : ""}>
              ...
              {stage.blocks.length > 0 ? (
                <div className="space-y-4">
                  {stage.blocks.map((block) => {
                    ...
                    const total = block.lotCount || 1; // Evitar división por cero
                    const percentages = {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mb-3 text-sm text-muted-foreground flex items-center gap-1">
                          <span>{block.lotCount} {block.lotCount === 1 ? "Lote" : "Lotes"}</span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-2 mt-3 text-xs">
                          <div className="flex items-center">
                            ...
                            <span>Activos: <span className="font-medium">{block.activeLots}</span></span>
                          </div>
                          <div className="flex items-center">
                            <div className="w-2.5 h-2.5 rounded-full bg-blue-500 mr-1.5" />
                            <span>Separados: <span className="font-medium">{block.reservedLots}</span></span>
                          </div>
                          <div className="flex items-center">
                            <div className="w-2.5 h-2.5 rounded-full bg-purple-500 mr-1.5" />
                            <span>Vendidos: <span className="font-medium">{block.soldLots}</span></span>
                          </div>
                          <div className="flex items-center">
                            <div className="w-2.5 h-2.5 rounded-full bg-muted-foreground/30 mr-1.5" />
                            <span>Inactivos: <span className="font-medium">{block.inactiveLots}</span></span>
                          </div>
                        </div>
                      </motion.div>
                    );
                ...
                <div className="text-center py-4 border rounded-md bg-card/50">
                  <p className="text-muted-foreground">Esta etapa no tiene manzanas registradas.</p>
                </div>
              )}
            </CardContent>
          </Card>
