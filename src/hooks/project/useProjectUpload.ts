...
export function useProjectUpload() {
  ...
  const [validationResult, setValidationResult] =
    useState<ExcelValidationResponse | null>(null);
  ...
  const validateExcel = async () => {
    ...
    try {
      ...
      if (result.isValid && result.data) {
        ...
        toast.error(
          "El archivo contiene errores. Por favor, revisa los detalles."
        );
      }
    } catch (error) {
  ...
  const createProject = async () => {
    if (!validationResult?.data) {
      toast.error("No hay datos válidos para crear el proyecto");
      return;
    }
    setIsCreating(true);
    try {
      await createBulkProject(validationResult.data);
