...
interface RegisterInitialPaymentApprovedModalProps {
  open: boolean;
...
export function RegisterInitialPaymentApprovedModal({
  open,
  onOpenChange,
  financingId,
  saleId,
  ...
  onSuccess,
}: RegisterInitialPaymentApprovedModalProps) {
  ...
  const exceedsLimit = totalAmount > initialAmountPending;
  ...
  const handleSubmit = async () => {
    ...
    if (totalAmount > initialAmountPending) {
      toast.error(
        `El monto total (${currencySymbol} ${totalAmount.toFixed(2)}) excede el monto pendiente (${currencySymbol} ${initialAmountPending.toFixed(2)})`
      );
      return;
    }
    ...
    try {
      ...
      await apiClient.post(
        `/api/sales/financing/initial-amount/paid-approved/${financingId}`,
        formData,
        {
          headers: {
      ...
      toast.success('Pago de inicial registrado exitosamente');
      queryClient.invalidateQueries({ queryKey: ['sale-detail', saleId] });
      queryClient.invalidateQueries({ queryKey: ['financing-detail'] });
      handleClose();
      onSuccess?.();
    } catch (error) {
      console.error(error);
      let message = 'Error al registrar el pago de inicial';
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="flex max-h-[90vh] flex-col sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>Registrar Pago de Inicial</DialogTitle>
          <DialogDescription>
            Registre un pago de inicial para el financiamiento. Este pago será aprobado
            automáticamente.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-3 gap-3 px-1 py-4">
          <div className="rounded-lg border p-3">
            <p className="text-muted-foreground mb-1 text-xs">Inicial Total</p>
            <p className="text-sm font-semibold">
              {currencySymbol} {initialAmount.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
            </p>
          </div>
          <div className="rounded-lg border p-3">
            ...
            <p className="text-sm font-semibold text-emerald-600">
              {currencySymbol}{' '}
              {initialAmountPaid.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
            </p>
          </div>
          <div className="rounded-lg border p-3">
            ...
            <p className="text-sm font-semibold text-amber-600">
              {currencySymbol}{' '}
              {initialAmountPending.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
            </p>
          </div>
        </div>
