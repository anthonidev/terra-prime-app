...
export function RegisterLateFeePaymentModal({
  open,
  onOpenChange,
  financingId,
  saleId,
  currency = 'PEN',
  title = 'Registrar Pago de Mora',
  pendingLateFee = 0,
  onSuccess,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.post(
        `/api/sales/financing/late-fees/paid-approved/${financingId}`,
        formData,
        {
          headers: {
      ...
      toast.success('Pago de mora registrado exitosamente');
      ...
      console.error(error);
      toast.error('Error al registrar el pago de mora');
    } finally {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="flex max-h-[90vh] flex-col sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Registre un pago para la mora pendiente del financiamiento. Este pago será aprobado
            automáticamente.
          </DialogDescription>
        </DialogHeader>

        {/* Pending Late Fee Info */}
        <div className="grid gap-4 px-1 py-4 sm:grid-cols-2">
          <div className="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-900 dark:bg-red-950">
            <p className="mb-1 text-xs text-red-700 dark:text-red-300">Mora Pendiente</p>
            <p className="text-lg font-bold text-red-700 dark:text-red-300">
              {currencySymbol}{' '}
              {pendingLateFee.toLocaleString('es-PE', { minimumFractionDigits: 2 })}
            </p>
          </div>
